...
- There are no GitHub Actions workflows in `.github/workflows`, and the main scheme has an empty autogenerated test action.

## Highest Priority Findings

### 1. No first-party verification baseline before risky refactors

Severity: High

Evidence:

- `AudioBooth/AudioBooth.xcodeproj/xcshareddata/xcschemes/AudioBooth.xcscheme:26-32` has a `TestAction`, but no test targets are listed.
- `.github/` contains funding and issue template files, but no CI workflow.
- `CONTRIBUTING.md:45-51` documents `swift-format`, but no build, lint, or test gate.

Impact:

Playback, auth, download, watch, and SwiftData changes currently rely on manual checks. That raises the risk of fixing one of the findings below while breaking another path, especially around persistence and security-sensitive auth flows.

Minimal fix:

Add a small CI workflow first: `xcrun swift-format lint --recursive .` plus the generic iOS `xcodebuild build` command used in this audit. Add package or app test targets after that for pure logic such as URL construction, auth redaction, and SwiftData recovery.

### 2. Storage cleanup runs expensive file and SwiftData work on the main actor
...
### 3. OIDC/auth logs can persist secrets
...
- `AudioBooth/AudioBooth/Services/AppLogger.swift:21-28` sets both stdout and persistent logging to debug, while only the persistent handler is wrapped by `RedactingLogHandler`.

Impact:

OIDC authorization codes, cookies, state values, and auth endpoint payloads can land in logs. Existing network redaction helps for structured Pulse network events, but these are ordinary app log messages, and stdout receives raw messages.
...
### 4. Shared connection links can export raw credentials
...
### 5. SwiftData save failures are swallowed after files are written
...
### 6. Download finalization can remove a valid previous file before replacement succeeds
...
- `AudioBooth/AudioBooth/Services/WatchConnectivityManager.swift:416-425` appends tokens as `?token=` for watch track URLs.
- `AudioBooth/AudioBooth Watch App/Models/WatchTrack.swift:8` stores the resulting URL on each watch track.
- `AudioBooth/AudioBooth Watch App/Models/LocalBookStorage.swift:21-23` persists encoded watch books to `UserDefaults`.
...
Query-string tokens are more likely to leak through logs, caches, screenshots, and copied URLs than headers. On watch download paths, the token-bearing URLs can also persist in encoded `WatchBook` data in `UserDefaults`.
...
2. Remove raw OIDC/cookie/response-body auth logging.
3. Move storage cleanup selection and file deletion off the main actor.
4. Make local model persistence failures propagate out of download completion.
5. Make download finalization atomic and size-checked.
